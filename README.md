...
> 当前发布版本：`0.14.0`（Alpha）。Windows 首次安装和后续启动都能在项目内自动补齐
...
- 每次制作都会保存歌词、音频、MV/封面、字体和选项为工程；重新打开时先询问继续上次工程
  还是新建空白工程，选择新建不会删除旧工程；恢复后即使网页文件控件丢失临时值，也会从
  工程清单补回素材，预览、校准与最终生成无需重复上传；
...
7. 使用 ±0.1 秒按钮时只微调首词开始或末词结束，句子中间已经对准的词完全不动；
