...
> 当前版本：`0.9.1`（Alpha）。建议先用一首 1～2 分钟的歌曲试跑并检查时间轴，再处理正式 MV。
...
- 为日语汉字添加振假名、为英文单词添加片假名读音，并可单独关闭英语自动注音；
...
推荐使用“一次上传、两阶段制作”：在“制作卡拉 OK MV”页上传 MV，并选择 Vmoe ASS、
UtaTen/QQ 音乐/网易云链接或自己的歌词；如果 MV 已含完整音轨，可以不再单独上传音频。先点
“生成可校准 KTV 工程”，
系统会基于独立音频或 MV 内嵌音轨生成/读取
时间轴、附加翻译、生成可编辑注音，并自动带入编辑器。逐句试听和微调后点击
“确认校准并开始制作 MV”，原音频和 MV 都会保留，并自动继续渲染成片；如果尚未选择
MV，制作页会保留校准结果并明确提示补选视频。这样既不用重复
...
- **制作卡拉 OK MV**：一次上传音频、MV 与本地/Vmoe/UtaTen/QQ 音乐/网易云歌词，先生成可校准工程，确认后再
