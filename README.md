...
> 当前版本：`0.12.2`（Alpha）。本版修复已登录网易云账号取得 SVIP 音质权限后仍可能下载失败的问题；校准与 MV 制作会优先使用兼容、稳定的标准音频，而不再强制下载体积巨大的母带文件。
...
- 未上传本地音频且 MV 没有音轨时，可明确选择已登录网易云的 Chrome、Edge、Firefox 或
  Brave，下载当前账号有权完整播放的标准音频流用于校准；不绕过权限，也不转换或解密 NCM；
