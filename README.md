...
> 当前发布版本：`0.12.4`（Alpha）。本版新增网易云一键登录和歌曲实景字幕预览：无需
> 关闭日常 Edge、使用 F12 或另装 Firefox，也能在生成前查看真实 MV/封面场景中的歌词。
...
- 未上传本地音频且 MV 没有音轨时，可一键打开 Karaoke Forge 专用的 Edge 官方登录窗口；
  日常 Edge 无需关闭，也不用 F12 或 Firefox；只获取账号有权播放的音频，不转换 NCM；
...
- 制作页会用歌曲的实际歌词预览字幕：有 MV 时叠加在对应时间的 MV 画面帧上，没有 MV 时
  使用当前专辑封面、背景主题和唱片布局；字体、字号、颜色与位置会即时更新；
...
当前封面与所选唱片/频谱场景，并明确提示成片中的旋转和波形动画。
