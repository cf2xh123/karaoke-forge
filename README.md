...
> 当前发布版本：`0.15.1`（Alpha）。Windows 首次安装和后续启动都能在项目内自动补齐
...
- 保留适合精修单句的逐句模式，并提供全局连续调整模式：整首音频只加载一次，可点击全曲
  句块后在下方继续微调、改字或删字；红色播放头会实时跟随播放，也可拖动连续跳转，KTV
  预览会自动跟句和逐词高亮，并可统一平移全局时间轴；
- 暂时隐藏不需要的歌词并保留恢复能力，或将错误行彻底删除；
- 由用户选择关闭、自动或强制逐字时间精修；
- 显示顶部中文翻译和左上、右下交替的传统 KTV 双行歌词；翻译与画面顶部的距离可调；
- KTV 双行歌词逐句滚动；当前句切换时，另一行会立即更新成再下一句预告；
- 歌曲开头或上下半段之间遇到长间奏时会清空歌词，并在下一句开唱前用三个逐级点亮的
  圆点倒数提示；可关闭提示或调整长间奏判定阈值；
...
8. 逐句模式继续用于仔细修正单句；切换到全局连续调整模式后，整首音频只加载一次，可在
   全曲时间轴点击任意句块或拖动红色播放头连续 seek；KTV 预览会跟随当前句和逐词时间，
   并可用全局平移统一修正整体偏移；
