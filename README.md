...
> 当前发布版本：`0.15.2`（Alpha）。Windows 首次安装和后续启动都能在项目内自动补齐
...
- 通过网易云、QQ 音乐或 UtaTen 链接创建工程时，名称留空会自动采用在线来源返回的正式
  歌名；手动填写的名称仍然优先；
...
- 保留网易云 LRC 中带时间的空白间奏行；清空普通歌词后可直接点击 `🗑 删除`；撤销作用于
  整份工程，不受当前播放句限制，并会恢复原文、回到被操作的行；
- 最终渲染前可边听当前句片段边用 ±0.1 秒微调首词开始/末词结束，已对准的句内词
  不会被整句缩放或平移；若延长后的句尾侵入下一句，可用默认开启的 Ripple（后句联动）
  顺延受影响的后句，也可关闭联动只改当前句；
- 保留适合精修单句的逐句模式，并提供全局连续调整模式：整首音频只加载一次，可点击全曲
  句块后在下方继续微调、改字或删字，也可直接拖动句块左右亮边修改句首/句尾；红色播放头
  会实时跟随播放，也可拖动连续跳转，KTV 预览会自动跟句和逐词高亮，并可统一平移全局时间轴；
- 暂时隐藏不需要的歌词并保留恢复能力，或将错误行彻底删除；
- 由用户选择关闭、自动或强制逐字时间精修；
- 显示顶部中文翻译和左上、右下交替的传统 KTV 双行歌词；翻译与画面顶部的距离可调；
- KTV 双行歌词逐句滚动；带时间的空白间奏行不会占用上下排，同排歌词即使时间有重叠也会
  在下一条同排歌词开始前安全收尾，不再被字幕渲染器挤成第三排或重复绘制；
- 歌曲开头或上下半段之间遇到长间奏时会清空歌词，并在下一句实际所在的歌词排上方显示
  独立的箭头胶囊，内部三个灯逐级点亮；可关闭提示或调整长间奏判定阈值；
- 为日语汉字添加振假名、为英文单词添加片假名读音；关闭英语注音后，旧工程或在线来源
  已经保存的英文读音也不会进入最终字幕；
- 可导入 TTF、OTF、TTC 自定义字体，不安装到系统也能随工程参与字幕渲染；
- 每次制作都会保存歌词、音频、MV/封面、字体和选项为工程；重新打开时工程列表会自动选中
  最近更新且仍可读取的工程，但不会未经确认覆盖当前页面。用户可一键载入制作页、载入并
  直接编辑，或保留当前制作页且不删除旧工程；自定义输出目录中的工程也会进入持久列表。
  恢复后即使网页文件控件丢失临时值，也会从工程清单补回素材，预览、校准与最终生成无需
  重复上传；
...
- Windows 可自动安装并校验项目私有 FFmpeg；最终字幕烧录会限制 libx264 线程数，避免特定
  Windows/FFmpeg 组合在长视频尾段发生原生崩溃；模型下载默认优先使用 ModelScope 魔搭国内
...
5. 上传歌曲、MV（或专辑图片）和歌词，先生成校准工程，试听确认后再生成最终视频；下次
   启动时工程列表会自动选中最近的有效工程，确认“载入所选工程”或“载入并直接编辑”后
   才会切换，不会未经确认覆盖当前页面。
...
网页的“歌词与注音编辑”可以载入 LRC、YRC、SRT、VTT、ASS、Karaoke Forge 歌词 JSON。
完整工程请从制作页顶部的已保存工程列表打开；浏览器单独上传 manifest 不会同时携带其
相邻素材，因此不作为工程恢复方式：

制作页顶部会列出仍可读取的已保存工程，并自动选中最近更新的一个；粘贴在线链接时还会
优先匹配同源工程。自动选中只改变候选项，不会立即覆盖当前页面；点击“载入所选工程”可
恢复制作素材，点击“载入并直接编辑”则会恢复后立即进入编辑器。列表可随时刷新；选择
“不载入”不会删除任何旧工程。

1. 推荐从制作页点“生成可校准 KTV 工程”，音频、歌词、翻译和自动注音会直接带入；
2. 打开“歌词总览”后，右键任意一行可以隐藏/显示、在上方或下方插入新行、删除；误操作
   可用全工程撤销恢复，即使播放已经切到下一句也会回到真正被操作的行；
3. 试听时可以开启“循环当前句”反复微调，关闭后唱完会自动跳过隐藏、空白或无时间轴行，
   播放下一句，并支持 0.5×–2× 倍速；焦点不在输入框时，按空格可暂停或继续；
...
7. 使用 ±0.1 秒按钮时只微调首词开始或末词结束，句子中间已经对准的词完全不动；若句尾
   延长后侵入下一句，默认开启的 Ripple（后句联动）会只向后平移新受影响的歌词，保持其
   句长和逐词间隔；关闭联动后则只修改当前句；
8. 逐句模式继续用于仔细修正单句；切换到全局连续调整模式后，整首音频只加载一次，可在
   全曲时间轴点击任意句块或拖动红色播放头连续 seek；拖动句块左侧/右侧亮边可精确修改
   该句句首/句尾，句内其他逐词时间保持不变，Ripple 开启时仍会联动后句；KTV 预览会跟随
   当前句和逐词时间，并可用全局平移统一修正整体偏移；
9. 在主表格中修改开始/结束秒、原文和翻译；
10. 点击“打开歌词总览”会从左侧滑出歌词抽屉；点击序号会选中、试听并自动收起，
   点击其他单元格则可留在抽屉中继续编辑；
   右键菜单和右侧“当前句操作”都能完成隐藏/显示或删除；
11. 隐藏行会保留在 JSON 中但不进入其他字幕或视频；删除则从导出项目中彻底移除；
12. 载入当前行注音，可修改整行读音或逐词读音表格；
13. 确认后交回制作页；原音频和 MV 保持不变，直接开始最终渲染。

逐词或注音修改尚未点“保存”时若歌曲自动进入下一句，编辑器会先保存本句并建立撤销快照；
随后点击“撤销 / 重做（全工程）”会回到发生修改的句子。删除、隐藏和插入同样记录整份
工程快照，不局限于当前播放句；没有可撤销内容时只会给出提示。当前句
...
当前行时长重新生成与新文本一致的逐字边界；直接修改总览表的行首/行尾会等比例缩放原边界，
而 ±0.1 秒按钮只改首词或末词边界。
