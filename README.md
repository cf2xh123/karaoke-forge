...
> 当前版本：`0.12.3`（Alpha）。本版支持 Edge/Chrome 保持开启时手动提供网易云 `MUSIC_U`，无需关闭浏览器或另装 Firefox；登录会话仅限网易云域名并强制使用 HTTPS。
...
- 未上传本地音频且 MV 没有音轨时，可选择已登录网易云的浏览器；Edge/Chrome 保持开启时
  也可手动提供 `MUSIC_U`，无需安装第二个浏览器；只获取账号有权播放的音频，不转换 NCM；
...
- Windows 上 Edge/Chrome 保持开启时，可从开发者工具复制 `music.163.com` 的 `MUSIC_U`
  Value 粘贴到密码框；手动值优先，程序不会读取被浏览器占用的 Cookie 数据库；
...
会员歌曲可以在网页中选择已登录网易云的浏览器；若 Edge/Chrome 正在运行，可按界面指引
只粘贴 `MUSIC_U`，不用关闭浏览器或改用 Firefox。也可以在网易云官方客户端中按平台许可
导出标准 MP3、FLAC、WAV 或 M4A 后上传。本项目：

- 不接收账号或密码；浏览器 Cookie 或手动提供的 `MUSIC_U` 只在本机内存中按需使用，
  不写入项目、输出目录或日志；
