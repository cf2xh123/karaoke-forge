...
def parse_ass(text: str) -> LyricsDocument:
    ...
    for raw in text.splitlines():
        ...
        if value:
            events.append((start, max(end, start + 0.01), style, value))
    ...
    lines = [
        LyricLine(text=value, start=start, end=end) for start, end, _style, value in lyric_events
    ]
    if not lines:
    ...
    if generated_by_karaoke_forge:
        for line in lines:
            translations = [
                value
                for start, end, style, value in events
